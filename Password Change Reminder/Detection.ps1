...
$PasswordExpirationDays = 60 ## Your Company Policy for Password -  how long is the Password valid
 <#
...
$ExpDate = $PasswordExpirationDate.ToString("dd-MM-yyy HH:mm:ss")
...
# Add password expiration date to registry HKCU:\Software\PasswordReminder\
$RegKey = Get-ItemPropertyValue -Path "REGISTRY::HKEY_USERS\$currentusersid\SOFTWARE\PasswordReminder\" -Name PasswordExpiration
if ($null -ne $RegKey){
    Remove-ItemProperty -Path "REGISTRY::HKEY_USERS\$currentusersid\SOFTWARE\PasswordReminder\" -Name PasswordExpiration
}
New-ItemProperty -Path "REGISTRY::HKEY_USERS\$currentusersid\SOFTWARE\PasswordReminder\" -Name PasswordExpiration -Value $ExpDate
